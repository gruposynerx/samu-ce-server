...
class UpdateRegionalGroupRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            'name' => [
                ...
                'string',
                'max:200',
                Rule::unique('regional_groups', 'name')->whereNot('id', $this->id),
            ],
            'bases' => ['required', 'array'],
            'bases.*' => [
                'uuid',
                Rule::exists('bases', 'id')->where(function ($query) {
                    $query
                        ->where('regional_group_id', null)
                        ->orWhere('regional_group_id', $this->id);
                }),
            ],
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'name.max' => 'O nome não deve ter mais que 200 caracteres.',
            ...
            'bases.*.uuid' => 'A base deve ser um uuid válido',
        ];
    }
}
